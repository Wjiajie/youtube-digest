...
test("built MV3 explicitly translates, restores one cloud attempt in both themes and discards late video replies", async ({}, info) => {
  const profile = await mkdtemp(resolve(".goal-loop/evidence/translation-mv3-profile-")), extension = resolve("apps/extension/.output/chrome-mv3");
  ...
  try {
    const id = (n: number) => `ef810000-0000-4000-8000-${String(n).padStart(12, "0")}`;
    ...
    let theme = "cyberpunk", defer = false, runId: string | null = null, generations = 0;
    const caption = { ownerId: owner, context: { bindingId: binding, nodeId: id(5), nodeTitle: "比较曝光", goalId: id(3), goalTitle: "用影像讲述生活", videoId },
      ...
      contentExpiresAt: "2026-09-11T00:10:00Z", title: "Exposure · Learn to see the light", language: "en", offset: 0, totalSegments: 20,
      segments: Array.from({ length: 20 }, (_, i) => ({ text: `${i + 1}. Observe the light and compare two examples before changing exposure.`, offsetMs: i * 15000, durationMs: 10000 })) };
    const receipt = () => ({ ok: true, run: runId ? { runId, accountId: owner, status: "ready", context: { bindingId: binding, videoId, sourceRunId: source, offset: 0 },
      targetLanguage: "zh-Hans", observedAt: caption.observedAt, contentExpiresAt: caption.contentExpiresAt,
      result: { status: "translated", providerMayHaveRun: true, usage: null, segments: caption.segments.map((_, segmentIndex) => ({ segmentIndex,
        translation: `第 ${segmentIndex + 1} 段：先观察光线，比较两个示例，再调整曝光。这是验证窄屏阅读的固定译文，不代表模型语义评测。` })) } } : null });
    await context.route("**/*", async route => {
      ...
      if (url.pathname === "/api/v1/translations/runs") {
        if (request.method() === "GET") return route.fulfill({ json: receipt() });
        const command = request.postDataJSON(); expect(command.accountId).toBe(owner); expect(command.sourceRunId).toBe(source);
        if (runId !== command.runId) generations++; runId = command.runId;
        return route.fulfill({ json: { ok: true, runId, status: "ready" } });
      }
      if (runId && url.pathname === `/api/v1/translations/runs/${runId}` && request.method() === "GET") {
    ...
    await panel.getByRole("tab", { name: "理解", exact: true }).click(); expect(generations).toBe(0);
    await panel.getByRole("button", { name: "读取原始字幕", exact: true }).click();
    await expect(panel.getByRole("button", { name: "翻译当前页", exact: true })).toBeEnabled(); expect(generations).toBe(0);
    await panel.getByRole("button", { name: "翻译当前页", exact: true }).click();
    await expect(panel.getByRole("button", { name: "译文已就绪", exact: true })).toBeVisible(); expect(generations).toBe(1);
    ...
    for (const next of ["cyberpunk", "eastern"]) {
      ...
      await expect(panel.getByRole("button", { name: "译文已就绪", exact: true })).toBeVisible();
      for (const width of [320, 1280]) {
        ...
        await expect(panel.locator(".transcript-bilingual li")).toHaveCount(20);
        await panel.screenshot({ path: info.outputPath(`${next}-translation-mv3-${width}.png`), fullPage: true });
      }
    }
    expect(generations).toBe(1);
    const stored = await worker.evaluate(async () => JSON.stringify(await chrome.storage.local.get(null)));
    expect(stored).not.toContain("Observe the light"); expect(stored).not.toContain("固定译文");
    defer = true; await panel.getByRole("button", { name: "核对当前翻译", exact: true }).click(); await expect.poll(() => Boolean(held)).toBe(true);
    ...
    await expect(panel.locator(".transcript-bilingual")).toHaveCount(0); expect(errors).toEqual([]);
