...
import { planningConfiguration } from "@/lib/agent/planning-runtime";
import { PlanningHistory } from "../../../planning/planning-history";
...
export default async function PlanningListPage({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ page?: string }> }) {
  ...
  const blueprint = brief.ok ? await createSupabaseBlueprintStore(client).getMainBlueprint(actor.userId).catch(() => null) : null;
  return <AccountThemeShell accountId={actor.userId} client={client}><main className="shell brief-shell planning-history">
    ...
    {records?.ok && brief.ok && blueprint ? <PlanningHistory start={{ accountId: actor.userId, briefId: id, briefRevision: brief.value.revision,
        blueprintVersion: blueprint.version, confirmed: brief.value.status === "confirmed", enabled: planningConfiguration() !== null }}
      heading={<header className="brief-intro"><div><p className="brand">让建议有迹可循</p><h1>规划记录</h1><p>{brief.value.content.outcome}</p></div><span className="brief-emblem" aria-hidden="true">策<span>PLANNING</span></span></header>}>
      <p className="subtle">按创建时间排列。进入记录核对当前状态，不会重新生成路径。</p>
      {records.value.runs.length ? <div className="brief-list">{records.value.runs.map((run, index) => <Panel key={run.id} className="brief-card"><p className="brand">PLANNING RECORD</p><h2>规划记录 {(page - 1) * 20 + index + 1}</h2><p><time dateTime={run.createdAt}>{new Date(run.createdAt).toISOString().replace("T", " ").slice(0, 19)} UTC</time></p><Link prefetch={false} href={`/planning/${run.id}`} className="bp-button">查看这次规划</Link></Panel>)}</div>
        : <Panel className="brief-card brief-empty"><h2>还没有规划记录</h2><p>目标定义已保留。只有你主动发起才会生成建议，这里不会用示例冒充你的路径。</p></Panel>}
    ...
    </PlanningHistory> : <Panel className="brief-card"><Status tone="warning">没有找到可访问的记录，或服务暂时不可用。不会创建空白规划替代已有数据。</Status><a href={path} className="bp-button">重新读取</a></Panel>}
