...
export async function readLearningTranscript(client: SupabaseClient, actor: Actor, input: unknown, signal?: AbortSignal): Promise<ApplicationResult<LearningTranscript>> {
  const parsed = learningTranscriptRequestSchema.safeParse(input);
  if (!parsed.success) return { ok: false, code: "invalid" };
  const request = parsed.data;
  try {
    signal?.throwIfAborted();
    const query = client.rpc("read_learning_transcript", { p_resource_binding_id: request.bindingId,
