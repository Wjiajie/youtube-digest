...
test("Web and verified extension recovery return safe errors without execution and reject invalid requests locally", async () => {
  const urls: string[] = [];
  ...
  expect(await createTranslationRunAccess(client, { userId: ownerId, client: "extension" }).read(runId)).toEqual({ ok: false, code: "not_found" });
  expect(await createTranslationRunAccess(client, { userId: "bad-owner", client: "extension" }).read(runId)).toEqual({ ok: false, code: "forbidden" });
  expect(urls.map(url => new URL(url).pathname)).toEqual(["/rest/v1/rpc/read_translation_run", "/rest/v1/rpc/cancel_translation_run", "/rest/v1/rpc/read_translation_run"]);
});
