...
it.each(["/api/planning/runs", "/api/clarification/turns", "/api/resources/runs", "/api/v1/learning-notes"])("lets the independently authenticated %s handler own its upload deadline", path => {
...
it.each(["/api/planning/runs/history", "/api/planning/runs-extra", "/api/clarification/turns/history", "/api/clarification/turns-extra",
  "/api/resources/runs/history", "/api/resources/runs-extra", "/api/v1/learning-notes/history", "/api/v1/learning-notes-extra"])("keeps the upload exception from widening to %s", path => {
