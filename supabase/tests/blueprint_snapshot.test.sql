...
select is(
  public.read_blueprint_snapshot('b5000000-0000-4000-8000-000000000001'),
  (select jsonb_build_object('schemaVersion', 1, 'id', id, 'version', 0, 'title', title, 'goals', '[]'::jsonb) from public.blueprints),
  'the new account reads one complete empty Blueprint snapshot'
);

create temporary table expected_snapshot as
select jsonb_build_object('schemaVersion', 1, 'id', id, 'version', 0, 'title', '真实路径',
  'goals', '[{"id":"b5000000-0000-4000-8000-000000000010","title":"演讲","description":"独立完成演讲","position":0,"stages":[{"id":"b5000000-0000-4000-8000-000000000020","title":"首周","position":0,"nodes":[{"id":"b5000000-0000-4000-8000-000000000030","title":"学习","description":"理解结构","position":0,"type":"learn","dependencyIds":[],"resources":[{"id":"b5000000-0000-4000-8000-000000000040","kind":"youtube_video","url":"https://www.youtube.com/watch?v=dQw4w9WgXcQ","externalId":"dQw4w9WgXcQ"}]},{"id":"b5000000-0000-4000-8000-000000000031","title":"练习","position":1,"type":"practice","dependencyIds":["b5000000-0000-4000-8000-000000000030"],"resources":[]},{"id":"b5000000-0000-4000-8000-000000000032","title":"检验","position":2,"type":"checkpoint","dependencyIds":[],"resources":[]},{"id":"b5000000-0000-4000-8000-000000000033","title":"复盘","position":3,"type":"reflection","dependencyIds":[],"resources":[]}]}]}]'::jsonb
...
select is(public.read_blueprint_snapshot('b5000000-0000-4000-8000-000000000001'), (select value from expected_snapshot),
  'all four node kinds, optional descriptions, ordered children, dependencies and resources survive a confirmed revision');
select is(public.read_blueprint_snapshot('b5000000-0000-4000-8000-000000000002'), null::jsonb,
  'the owner argument cannot select another user');
select is(public.read_blueprint_snapshot(null), null::jsonb, 'NULL owner never broadens a read');

select set_config('request.jwt.claims', '{"sub":"b5000000-0000-4000-8000-000000000002"}', true);
select is(public.read_blueprint_snapshot('b5000000-0000-4000-8000-000000000001'), null::jsonb,
  'another authenticated account cannot read the owner snapshot');
select is(public.read_blueprint_snapshot('b5000000-0000-4000-8000-000000000002')->'goals', '[]'::jsonb,
...
set local role anon;
select throws_ok($$select public.read_blueprint_snapshot('b5000000-0000-4000-8000-000000000001')$$,
  '42501', 'permission denied for function read_blueprint_snapshot', 'anonymous RPC execution is denied');
reset role;
select ok((select not prosecdef and provolatile = 's' from pg_proc where oid = 'public.read_blueprint_snapshot(uuid)'::regprocedure),
