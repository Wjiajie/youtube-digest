...
后续[目标上下文增量](extension-learning-context.md)已本地接入节点原始说明、完成依据、预计投入与定位原节点的返回入口。同视频有多绑定时必须明确选择；切标签页／视频重新核对，读取期间和失败后不沿用旧上下文动作。两套紧凑主题共用业务语义，未把未实现的字幕／理解界面画成可用功能。
