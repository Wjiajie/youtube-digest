...
2026-09-11，固定点 `cde189f`。继续核查“形态键是否能携带衣袍姿态修正”，没有继续盲调静止腰口。本批只在固定 v17 的内存副本上，为四片真实袍片添加最大 40 mm 的校准位移，保持上沿不动；**校准位移不是服装修复方案**。未保存新 `.blend`、未修改正式配方、生产加载器或首页；所有试验文件保留在忽略目录。
