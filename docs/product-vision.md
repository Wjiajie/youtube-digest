...
学习节点可以绑定规范的 YouTube 视频链接。M1 扩展已经能匹配当前视频并由用户明确开始一条学习会话；后续本地增量已接入[私人笔记](note-workspace.md)和[继续学习位置](position-workspace.md)，尚未托管发布。字幕获取、视频概览、翻译和选文讲解仍待重新接入。规划结果因此不是静态图，而是实际学习行为的入口。
