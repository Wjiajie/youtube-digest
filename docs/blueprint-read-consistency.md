...
更新：2026-09-10；固定点 `db27ece`。源码和本地数据库已接入，**本批没有托管迁移或发布**。这是 P4 目标定义、投入与完成标准扩展前的可靠性前置，不代表新字段已经交付。
...
- **后续必须先应用迁移，再部署依赖它的 Web。** 缺少 RPC 时明确不可用，不使用旧读取作降级。当前生产 `c0bcf45` 仍用旧实现，本地修复尚未线上生效；回滚 Web 不要求删函数或回滚数据。
...
常规生产模式 E2E 20 项通过（7.5 秒），覆盖登录恢复、匿名私密接口和内部设计页门禁。独立规范／需求审阅待本批收口补充。UI／3D 本批未改；正式双主题品质、真实托管旅程和其余 P1–P9 保持待办。
