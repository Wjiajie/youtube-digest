...
2026-09-11 来源链增量：[具体资源包授权核验](asset-license-provenance.md)补到 Women、Nature、Cyberpunk 三篇作者发布帖的明确 CC0／商用声明，并核对同一下载目录、所选文件条目与本地原件哈希。三份 TXT 错标题仍存在；不能继续概括为“只有首页 CC0”，也不以此自动放行分发。下文旧接收记录保留当时事实。

最新制作切片：[八影响完整导出与内部试装](eight-influence-preview.md)。修复源新导出 `Casual.full-influence-study.glb`，2,003,284 字节，SHA-256 `7892fef0124addd693b11a5181743c718b61ef979f86e53800e7d26d39984d96`；源件和既有导出保持不变。镜像后最高八影响，真实 GPU／四个人物旅程已验证；全动作保真、烘焙警告、许可对应和正式美术仍待验收。

最新修复：A2 的四处越界权重已在新源工程中修正，导出文件与先前试样逐字节一致；多于四骨骼的适配尚未通过，详见末节。没有修改下载原件或把诊断进展当作美术完成。
...
## 2026-09-10 A2 权重修复与兼容性分界

固定点 `2bb631c`。用[源数据检查](../scripts/check-casual-source-quality.py)复现红灯：Blender 将 Body 三处、Legs 一处大于 1 的权重判为无效网格，不是面片损坏；Body／Head／Legs 分别有 95／10／16 个顶点超过四骨骼影响，最高七项。检查仅在临时 mesh 副本上调用 validate，下载原件不变。

[受限修复](../scripts/repair-casual-source-weights.py)只将已核实的 `(Body,280,19)`、`(Body,290,19)`、`(Body,406,23)`、`(Legs,31,56)` 四个顶点／组的权重设为 1，保存新工程 `Casual.weight-repaired-study.blend`，不删减任何影响、不改造型或保存原件。新文件为 5,543,843 字节，SHA-256 `216fc727182f827c9774d862e2f161c58d3d7e63367f43ad9c3984d386cc0395`；容器由 Blender 4.5 保存，不承诺与旧 2.79 工程除四值以外逐字节一致。

[独立重新打开检查](../scripts/check-casual-weight-repair.py)确认恰好这四值变化，影响分配集合、其余权重、顶点坐标、面索引、UV、修改器类型和骨骼端点均不变。检查脚本最初把同一列表交给 Blender 加载器，列表被原地替换成 Object 导致第二次加载失败；改为不可变名称元组与每次独立列表后通过，未因此改变修复文件。

修复前 `check-casual-source-quality.py -- --mesh-only` 失败，修复后通过；**不带此参数的四骨骼检查仍按预期失败**，121 个超限顶点保持原样。这是两个独立门槛，不是把未解决检查改成绿色。以上脚本都使用此前记录的 factory／禁止自动脚本／离线选项及 `--python-exit-code 1`；独立重新打开检查不需要在命令行先加载文件，其余检查加载原件或修复件。

### 为什么没有直接裁掉多余权重

前四项之外的最大权重占比约 12.3%。[内存变形探针](../scripts/probe-casual-influence-reduction.py)对 24 个动作每五帧取样，共 174 个姿态，按顺序比较“原数据→只修越界值→再截前四项并归一化”：只修越界值的最大位移约 `1.69e-7` 源世界单位；额外截断在 Kick_Right 第 10 帧达到 `0.025885`，中性待机达到 `0.005593`。这是采样几何差异，不是人体毫米标定、完整逐帧检查或人工可感知阈值。探针不写文件；裁剪方案未保存到源工程。

现有 Three.js GLTFLoader／蒙皮材质使用 JOINTS_0／WEIGHTS_0，作者原 glTF 和既有四骨骼导出本就只带这一组。因此“现有浏览器会播放”不证明完整源变形得以保留。多影响适配或经审阅的重新配重仍需实现，不通过隐藏警告宣称兼容完成。
