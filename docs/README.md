...
26. [规划记录与草案阅读](agent-planning-review.md)：本地双主题记录列表、草案阅读、当前状态核对与取消；生成入口和正式应用未开放。
...
P6 已有内部路径规划器、版本化规划 Skill、持久运行记录和本地双主题阅读／取消页面，但尚未开放生成入口、真实模型服务或规划正式应用，也未托管发布。M1 仍没有完整用户可用的 Agent 规划、正式 3D、字幕工作台、推荐或复盘闭环。这些必须以路线状态为准，不能当作当前能力宣传。
