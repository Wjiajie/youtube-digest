...
23. [双主题人物与场景制作](dual-theme-character-study.md)：v14 贴合服装／空间轮廓与共享光照、真实首页双主题构图证据；内部样片不等于正式美术通过。
...
49. [学习位置云端保存](learning-positions.md)：跨客户端位置读写、并发冲突与原请求恢复；继续学习界面和播放器接入仍待实现，不自动推断进度。
