...
10. [真实资产接收台账](asset-intake.md)：实际下载文件、SHA-256、许可差异与尚未通过的接入门槛。
    - [具体资源包授权来源](asset-license-provenance.md)：作者包级 CC0 声明、所选文件来源链与仍未解决的 TXT 标题差异；不自动放行分发。
...
23. [双主题人物与场景制作](dual-theme-character-study.md)：v15 动作修复与腰口诊断、未采用的 v16 腰带试验；内部样片不等于正式美术通过。
...
32. [按节点的视频候选检索](resource-discovery.md)：真实供应商适配、可用性筛选与少量原生字幕；本地服务端切片，后续已接入审阅与采用流程。
...
44. [私人时间戳笔记](learning-notes.md)：云端原文、历史节点／视频归属、精确提交恢复与共享读写入口；云端基础批次，后续界面见双端笔记工作台。
45. [双端笔记工作台](note-workspace.md)：Web／扩展共享表单、原提交恢复、独占编辑、两主题私人历史与手动位置链接；无自动播放追踪。
46. [明确读取播放位置](player-position.md)：扩展主动读取同视频秒数、原生媒体与页面桥接核验、失败保留草稿；不是自动观看追踪或掌握证明。
47. [响应压缩监听器修复](next-compression-repair.md)：Next 16.3.3 背压监听器根因、精确依赖补丁、HTTP 回归与升级移除要求；本地验证，不代表托管或完整性能验收。
48. [明确清除资源证据](resource-evidence-clearing.md)：主动确认清理整条资源链、保留不可重复执行回执与用户学习历史；本地增量，不是自动到期、账号或处理商删除。
    - [资源证据期限与到期恢复](resource-evidence-expiry.md)：可信来源期限、派生副本继承、到期访问保护和无正文回执；持续托管调度与内容授权仍待完成。
    - [资源维护与健康检查](resource-maintenance.md)：限批执行回执、无正文积压指标及独立 stdin 检查；已做本地临时调度演练，未安装持续监控。
49. [学习位置云端保存](learning-positions.md)：跨客户端位置读写、并发冲突与原请求恢复；保留云端基础批次的证据与边界。
...
51. [双端原始字幕阅读](learning-transcript.md)：Web 路径与扩展“理解”共用原文分页、固定来源和到期隐藏；仅读取已获取材料，不自动消费或推断进度。
    - [双语字幕翻译与执行链](transcript-translation.md)：来源绑定、持久恢复、独立核验执行、精确扩展授权及双主题双端阅读；本地已接通，托管与完整质量仍待验收。
    - [来源绑定的选文讲解](selection-explanation.md)：双端共享双主题选文／提问、有限原文证据、材料不足与明确恢复；内存草稿、云运行、取消／清除及关闭开关体验已本地验证，原生侧栏与托管联合验收待完成。
...
P7 已有按节点发现视频、原生字幕、受控内容匹配、持久执行链、[生产形态接口／资源 worker](agent-resource-entry.md)、[双主题资源工作台](resource-workbench.md)及[明确采用／替换](resource-adoption.md)；真实本地账号／数据库与第三方 HTTP 夹具已联调。开关默认关闭，真实服务及托管启用尚未完成，不代表真实推荐服务已经开放。
