...
25. [路径规划的云端运行记录](agent-planning-runs.md)：本地已实现账号隔离、唯一执行权、次数保护、取消与过期结果恢复；用户入口、付费模型及托管发布仍待完成。
...
P6 已有内部路径规划器、版本化规划 Skill 和经过真实本地账号／数据库验证的持久运行记录，但尚未接入用户页面或真实模型服务，也未托管发布。M1 仍没有用户可用的 Agent 规划、正式 3D、字幕工作台、推荐或复盘闭环。这些必须以路线状态为准，不能当作当前能力宣传。
