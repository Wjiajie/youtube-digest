...
27. [规划草案的提案确认](agent-planning-approval.md)：本地两步确认、不可替换的来源绑定、原子来源检查、拒绝与精确恢复；真实生成入口和托管发布仍待完成。
...
P6 已有内部路径规划器、版本化规划 Skill、持久运行记录和本地双主题阅读／取消／提案确认页面，但尚未开放生成入口或真实模型服务，也未托管发布。M1 仍没有完整用户可用的 Agent 规划、正式 3D、字幕工作台、推荐或复盘闭环。这些必须以路线状态为准，不能当作当前能力宣传。
