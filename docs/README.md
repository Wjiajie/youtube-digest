...
30. [澄清云端会话](agent-clarification-sessions.md)：独立工作摘要、明确保存、次数与执行权、真实 Edge／数据库恢复；本地已验证，对话页面与托管发布尚未接入。
...
P6 已有版本化规划 Skill、持久运行记录、本地主动生成入口及双主题阅读／取消／提案确认页面。生成需要显式服务端配置和账号次数，默认关闭；真实模型消费、质量评测和托管发布未验收。M1 仍没有完整用户可用的 Agent 规划、正式 3D、字幕工作台、推荐或复盘闭环。这些必须以路线状态为准，不能当作当前能力宣传。
