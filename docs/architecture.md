...
P7 [视频候选检索](resource-discovery.md)已有独立内部服务端模块：从调用者提供的蓝图学习节点构造有限查询，固定 YouTube／Supadata 来源，筛选元信息并检查最多三条原生字幕，不写蓝图、不读取环境密钥、不开放路由。[资源匹配 Skill](agent-resource-matching.md)已接入内部候选证据，检查来源修订、有限 ID 和字幕引文，只产生待审阅建议。来源账号验证、从数据库读取当前版本、持久任务与账号额度仍由后续受控调用层实现，结构校验不能替代授权。当前仅本地 HTTP 夹具验收，尚无推荐 UI 或真实供应商消费。
