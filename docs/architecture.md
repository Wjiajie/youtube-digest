...
P6 现有一个尚未接入页面／API 的[内部路径规划器](agent-path-planning.md)：固定版本 Skill 加 AI SDK 结构化输出，接收已确认定义和蓝图快照，只返回隔离草案，不持有数据库写入或检索工具。来源修订、周安排与用量一并返回，未来须由已认证云端运行保存检查点、预留／核对额度并再次校验版本；当前不提供真实模型服务，也不代表上述 M1 运行链路已包含 Agent。
