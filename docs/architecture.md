...
P8 [双端继续学习工作台](position-workspace.md)在本地共用来源绑定的 UI、位置读写和精确回执。路径版本确认节点／资源，独立位置版本防止跨设备覆盖；浏览器草稿仅负责恢复，云端记录是已保存位置的事实来源。扩展仅在用户明确要求时读取当前视频位置，不新增权限或自动观看追踪；账号变化隐藏内容、迟到响应隔离，正式蓝图和学习证据不因位置保存而改变。

P8 [双端原始字幕阅读](learning-transcript.md)新增当前绑定下的窄只读投影。Web 与精确配置的扩展 OAuth 身份读取本人有效材料的有限分页，不授予扩展原始资源运行表或完整模型输入权限。共享组件保留来源、原文和语言，到期或恢复窗口时隐藏旧正文；首次 SSR 不含正文，不写浏览器存储，不触发提供方、续期或进度保存。
...
- 读取私人状态工作区并明确确认节点自评；成果与自评使用同一账号绑定传输，但各自保存与恢复原请求，不进入会话 outbox，见[扩展节点自评](extension-node-assessment.md)；
- 写入受限的授权和同步结果事件。

用户打开 YouTube watch 页面时，扩展按规范视频 ID 匹配全部 Resource Binding；多绑定时由用户选择，单绑定才自然展示。开始会话和返回原路径前，后台重新验证账号、活动标签页及所选绑定，不能信任调用方提供的标题或任意 URL。节点说明、完成依据与预计投入取自正式快照；旧格式缺失时不虚构。详见[扩展目标上下文](extension-learning-context.md)。只有点击“开始学习”才创建会话，不根据打开页面或观看时长推断学习。
