...
P7 [视频候选检索](resource-discovery.md)已有独立内部服务端模块：从调用者提供的蓝图学习节点构造有限查询，固定 YouTube／Supadata 来源，筛选元信息并检查最多三条原生字幕，不写蓝图、不读取环境密钥、不开放路由。来源账号验证、当前版本检查、持久任务与账号额度仍由后续受控调用层实现，结构校验不能替代授权。当前仅本地 HTTP 夹具验收，尚无匹配 Skill、推荐 UI 或真实供应商消费。
