...
P4 已增加独立于正式路径的 Goal Brief：规划前定义支持不完整草稿、明确确认、修改后重新确认、独立修订及精确请求回执。仅 Web 当前账号可访问，确认定义不修改 BlueprintSnapshot；未来 Agent 仍需单独校验确认修订并生成提案。当前只有本地域模型／数据库／Action，没有卡片 UI、Agent 或托管发布，详见[目标定义云端基础](goal-briefs.md)。
