...
P6 现有一个尚未接入页面／API 的[内部路径规划器](agent-path-planning.md)：固定版本 Skill 加 AI SDK 结构化输出，接收已确认定义和蓝图快照，只返回隔离草案，不持有数据库写入或检索工具。[云端运行编排](agent-planning-runs.md)已在本地连接真实 Auth／PostgREST：用户身份读取真实来源并预留一次额度，服务端领取唯一执行权，在事务之外调用规划器，保存完整结果及 Skill 并复核来源修订。取消、超时、并发及响应丢失均不能自动重跑模型；新账号默认零次，次数保护不等于人民币账务。用户只能恢复自己的运行，扩展不能调用规划，模型仍不能修改正式蓝图。当前不提供用户入口或真实模型服务，也未托管发布，不代表上述 M1 运行链路已包含 Agent。
