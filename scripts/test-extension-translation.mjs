...
try {
  if (args.some(arg => arg === "-c" || arg.startsWith("-c=") || arg === "--config" || arg.startsWith("--config=")))
    throw new Error("The local translation test configuration cannot be overridden");
  ...
  const created = await admin.auth.admin.oauth.createClient({ client_name: `Translation HTTP fixture ${randomUUID()}`,
    redirect_uris: ["http://127.0.0.1:54329/extension-translation-http-fixture"], scope: "email", token_endpoint_auth_method: "none" });
  ...
  exitCode = await new Promise((resolve, reject) => {
    child = spawn("npx", ["playwright", "test", "--config", "playwright.translations.config.ts", ...args], {
      cwd, env: { ...process.env, BLUEPRINT_TRANSLATION_TEST_EXTENSION_CLIENT_ID: clientId }, stdio: "inherit",
    });
  ...
  console.error("Local extension translation launcher failed; no hosted services were used.");
