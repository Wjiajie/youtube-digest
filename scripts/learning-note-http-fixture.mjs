...
export default async function setup() {
  const records = new Map(), positions = new Map(), calls = [];
  const server = createServer(async (request, response) => {
    ...
    if (request.url === "/fixture/reset" && request.method === "POST") { records.clear(); positions.clear(); calls.length = 0; return send(200, { ok: true }); }
    ...
    if (request.url === "/rest/v1/rpc/record_learning_position") {
      const row = { id: "fd570000-0000-4000-8000-000000000005", owner_id: noteFixtureOwner, client_mutation_id: input.p_client_mutation_id,
        ...
        video_id: "abcdefghijk", resource_url: "https://www.youtube.com/watch?v=abcdefghijk",
        expected_position_version: input.p_expected_position_version, position_version: input.p_expected_position_version + 1,
        position_seconds: input.p_position_seconds, created_at: "2026-09-11T00:00:00.000Z" };
      positions.set(input.p_resource_binding_id, row); return send(200, row);
    }
    if (request.url === "/rest/v1/rpc/read_learning_position_workspace" && input.p_owner_id === noteFixtureOwner) return send(200, {
      blueprint: { schemaVersion: 2, id: "fd510000-0000-4000-8000-000000000006", version: 2, title: "继续学习路径", goals: [] },
      records: [...positions.values()].filter(row => !input.p_resource_binding_id || row.resource_binding_id === input.p_resource_binding_id),
    });
